...
def extract_feat(l_utt):
    ...
    for line in l_utt:
        ...
        #srec_power,spec_power_en=pse(utt,sample_rate,nfft)              #(nfft,)(1,)
        #spec_power_en = np.atleast_1d(spec_power_en)
        signal_flu=np.diff(utt)
        #print(srec_power.shape)
        #print(spec_power_en.shape)
        #提取多尺度排列熵
        mpe=MSE(signal_flu, scale)    #(scale,)
        mpe = np.array(mpe)
        ...
        #特征融合
        #fusion=[mfcc.reshape(1,-1),srec_power,spec_power_en,mpe]
        #fusion = np.concatenate((spec.flatten(),mpe),axis=0)
        
        
        #print(fusion.shape)
        
        fusion = np.expand_dims(mpe, axis = 0).astype(np.float32)		# add 0 dim for torch
        print(fusion.shape)
        ...
        np.save(_dir_name+fn, fusion)

    return

_dir_dataset = '/data4/mahaoxin/data/ASVspoof2019/LA/dev/wav/'
_dir_name = '/data4/mahaoxin/data/ASVspoof2019/LA/dev/mse/'
_nb_proc = 8
nfft=256
scale=20
